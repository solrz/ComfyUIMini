...
<template>
    <div class="size-full overflow-y-auto flex flex-col gap-2">
        <div @click.stop="$router.go(-1)"
            class="w-full bg-surface text-white p-6 rounded-xl text-center text-lg cursor-pointer">
            Back
        </div>
        ...
        <div class="flex flex-col gap-2">
            <div v-for="prompt in historyReversed" :key="prompt.id" class="bg-surface p-2 rounded-xl"
                ...
                <div v-for="(outputNodeImages, outputNode) of prompt.prompt.outputs" :key="outputNode"
                    class="bg-surface-light p-2 rounded-lg">
